...
import { IRequestWithDecodedAuthToken } from 'src/libs/types/interfaces/utils.interfaces';
...
export class RoleGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    let cachedRole: string | number | null = await this.redis.get(cacheKey);
    ...
    if (!cachedRole) {
      ...
      await this.redis.set(cacheKey, auth.roleLevel ?? 0, 'EX', 60 * 60); // 1 hour
